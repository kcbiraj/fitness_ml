...
predictor_columns = list(df.columns[:6])  # Exclude the first column (timestamp)
...
# print(f"Set {s} duration: {duration.seconds} seconds")
# --------------------------------------------------------------
# Butterworth lowpass filte1
# --------------------------------------------------------------
...
for col in predictor_columns:
    ...
    # df_temporal = NumAbs.abstract_numerical(df_temporal, [col], ws, "max")
    # df_temporal = NumAbs.abstract_numerical(df_temporal, [col], ws, "min")
...
# Visualize the frequency features
...
# Plot acceleration data with clusters
fig = plt.figure(figsize=(20, 15))
ax = fig.add_subplot(111, projection="3d")
